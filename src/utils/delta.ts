...
import { DeltaInsert, InsertDelta, SetDelta } from '../model';
...
export function normalizeSetDelta(delta: SetDelta): SetDelta {
  const normalized: SetDelta = [];

  for (const element of delta) {
    if (typeof element.set === 'string' && element.set.length === 0) {
      continue;
    }
    ...
    if (
      !prev ||
      typeof prev.set !== 'string' ||
      typeof element.set !== 'string'
    ) {
      normalized.push(element);
      continue;
    }
